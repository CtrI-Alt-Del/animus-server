...
class ExtractPetitionJob(InngestJob):
    @staticmethod
    def handle(inngest: Inngest) -> Any:
        @inngest.create_function(
            fn_id='extract-petition',
            trigger=TriggerEvent(event=PetitionExtractionRequestedEvent.name),
            retries=0,
            on_failure=ExtractPetitionJob._handle_failure,
        )
        async def _(context: Context) -> None:
            ...
            normalized_data = await context.step.run(
                'normalize_payload',
                ExtractPetitionJob._normalize_payload,
                data,
            )
            ...
            try:
                summary_result_data = await context.step.run(
                    ...
                    lambda payload=payload: ExtractPetitionJob._extract_and_summarize(
                        payload
                    ),
                )
                ...
                await context.step.run(
                    ...
                    lambda payload=payload: (
                        ExtractPetitionJob._mark_petition_as_not_found(payload)
                    ),
                )
            except Exception:
                await context.step.run(
                    'mark_analysis_as_failed',
                    lambda payload=payload: ExtractPetitionJob._mark_analysis_as_failed(
                        payload
                    ),
                )
                raise
    ...
    @staticmethod
    async def _extract_and_summarize(payload: _Payload) -> dict[str, str]:
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            None,
            lambda: ExtractPetitionJob._extract_and_summarize_sync(payload),
        )

    @staticmethod
    def _extract_and_summarize_sync(payload: _Payload) -> dict[str, str]:
    ...
    async def _mark_petition_as_not_found(payload: _Payload) -> None:
        ...
        await loop.run_in_executor(
            None,
            lambda: ExtractPetitionJob._mark_petition_as_not_found_sync(payload),
        )
    ...
    def _mark_petition_as_not_found_sync(payload: _Payload) -> None:
        with Sqlalchemy.session() as session:
            ...
            UpdateAnalysisStatusUseCase(
                ...
                analysis_id=analysis_id.value,
                status=SecondInstanceAnalysisStatus.PETITION_NOT_FOUND.value,
            )
            session.commit()
    ...
    async def _mark_analysis_as_failed(payload: _Payload) -> None:
        ...
        await loop.run_in_executor(
            None,
            lambda: ExtractPetitionJob._mark_analysis_as_failed_sync(payload),
        )
    ...
    def _mark_analysis_as_failed_sync(payload: _Payload) -> None:
        with Sqlalchemy.session() as session:
            ...
            UpdateAnalysisStatusUseCase(
                ...
                analysis_id=analysis_id.value,
                status=SecondInstanceAnalysisStatus.FAILED.value,
            )
            session.commit()

    @staticmethod
    async def _handle_failure(context: Context) -> None:
        event_data = ExtractPetitionJob.get_event_data_from_context_failure(context)
        normalized_data = await ExtractPetitionJob._normalize_payload(event_data)
        payload = _Payload(analysis_id=str(normalized_data['analysis_id']))

        await ExtractPetitionJob._mark_analysis_as_failed(payload)
