...
class SummarizeCaseJob(InngestJob):
    @staticmethod
    def handle(inngest: Inngest) -> Any:
        @inngest.create_function(
            ...
            retries=0,
            on_failure=SummarizeCaseJob._handle_failure,
        )
        async def _(context: Context) -> None:
            ...
            normalized_data = await context.step.run(
                'normalize_payload',
                SummarizeCaseJob._normalize_payload,
                data,
            )
            ...
            try:
                summary_result_data = await context.step.run(
                    'summarize_case',
                    lambda payload=payload: SummarizeCaseJob._summarize_case(payload),
                )
    ...
    async def _summarize_case(payload: _Payload) -> dict[str, str]:
        ...
        return await loop.run_in_executor(
            None,
            lambda: SummarizeCaseJob._summarize_case_sync(payload),
        )
    ...
    def _summarize_case_sync(payload: _Payload) -> dict[str, str]:
        with Sqlalchemy.session() as session:
            ...
            workflow = AgnoSummarizeCaseWorkflow(
    ...
    async def _mark_analysis_as_failed(payload: _Payload) -> None:
        ...
        await loop.run_in_executor(
            None,
            lambda: SummarizeCaseJob._mark_analysis_as_failed_sync(payload),
        )
    ...
    async def _handle_failure(context: Context) -> None:
        event_data = SummarizeCaseJob.get_event_data_from_context_failure(context)
        normalized_data = await SummarizeCaseJob._normalize_payload(event_data)
        payload = _Payload(analysis_id=str(normalized_data['analysis_id']))

        await SummarizeCaseJob._mark_analysis_as_failed(payload)
