...
class SendPrecedentsSearchFinishedNotificationJob:
    ...
    def handle(inngest: Inngest) -> Any:
        @inngest.create_function(
            fn_id='send-precedents-search-finished-notification',
            trigger=TriggerEvent(
                event=PrecedentsSearchFinishedEvent.name,
            ),
        )
        async def _(context: Context) -> None:
            ...
            await context.step.run(
                ...
                lambda payload=payload: (
                    SendPrecedentsSearchFinishedNotificationJob._send_notification(
                        payload
                    )
                ),
            )
    ...
    async def _send_notification(payload: _Payload) -> None:
        ...
        await loop.run_in_executor(
            None,
            lambda: SendPrecedentsSearchFinishedNotificationJob._send_notification_sync(
                payload
            ),
        )
