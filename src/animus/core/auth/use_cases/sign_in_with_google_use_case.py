...
from animus.core.auth.domain.structures.social_account_provider import SocialAccountProviderValue
...
class SignInWithGoogleUseCase:
    def __init__(self, accounts_repository: AccountsRepository, jwt_provider: JwtProvider, google_oauth_provider: OAuthProvider):
    ...
    def execute(self, id_token: str) -> SessionDto:
        ...
        account = self._accounts_repository.try_to_find_by_email(email)
        ...
        if not account:
            ...
            social_account = SocialAccount.create(SocialAccountDto(provider='GOOGLE',name=account.name.value,email=account.email.value))
