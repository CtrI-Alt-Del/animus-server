...
from animus.core.intake.use_cases import ListAnalysisDocumentsUseCase
from animus.pipes.database_pipe import DatabasePipe
from animus.pipes.intake_pipe import IntakePipe


class ListAnalysisDocumentsController:
    @staticmethod
    def handle(router: APIRouter) -> None:
        @router.get(
            '/analyses/{analysis_id}/documents/all',
            status_code=200,
            response_model=list[AnalysisDocumentDto],
        )
        def _(
            analysis: Annotated[
                Analysis,
        ...
        ) -> list[AnalysisDocumentDto]:
            use_case = ListAnalysisDocumentsUseCase(
