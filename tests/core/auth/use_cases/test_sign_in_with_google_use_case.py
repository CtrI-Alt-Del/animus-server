...
from animus.core.auth.use_cases.sign_in_with_google_use_case import (
...
class TestSignInWithGoogleUseCase:
    ...
    def setup(self) -> None:
        ...
        self.mock_access_token = TokenDto(
            value='access_token', expires_at='2026-12-31T23:59:59Z'
        )
        self.mock_refresh_token = TokenDto(
            value='refresh_token', expires_at='2026-12-31T23:59:59Z'
        )
        self.expected_session = SessionDto(
            access_token=self.mock_access_token, refresh_token=self.mock_refresh_token
        )
    ...
    def test_should_create_account_and_link_social_when_account_does_not_exist(
        ...
        self.google_oauth_provider_mock.get_user_info.assert_called_once_with(
            Text.create(oauth_token)
        )
        self.accounts_repository_mock.find_by_email.assert_called_once_with(email)
    ...
    def test_should_link_social_account_and_replace_when_account_exists_but_not_linked(
        ...
        existing_account_mock.id.value = account_id
        existing_account_mock.name.value = name.value
        existing_account_mock.email.value = email.value
        ...
        self.google_oauth_provider_mock.get_user_info.return_value = (name, email)
        self.accounts_repository_mock.find_by_email.return_value = existing_account_mock
        self.jwt_provider_mock.encode.return_value = self.mock_encoded_result
        ...
        self.accounts_repository_mock.replace.assert_called_once_with(
            existing_account_mock
        )
        self.accounts_repository_mock.add.assert_not_called()
        self.jwt_provider_mock.encode.assert_called_once_with(Text.create(account_id))
        assert result == self.expected_session
