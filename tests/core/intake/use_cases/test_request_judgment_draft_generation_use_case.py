...
class TestTriggerSecondInstanceJudgmentDraftGenerationUseCase:
    ...
    def test_should_publish_event_when_there_is_at_least_one_chosen_precedent(
        ...
        assert isinstance(event, SecondInstanceJudgmentDraftGenerationTriggeredEvent)
    ...
    def test_should_raise_when_analysis_is_not_second_instance(self) -> None:
        analysis_id = Id.create().value
        analysis = self._create_analysis(
            analysis_id, AnalysisType.create_as_first_instance().dto
        )
        self.analisyses_repository_mock.find_by_id.return_value = analysis
