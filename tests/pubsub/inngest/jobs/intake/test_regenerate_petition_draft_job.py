...
from animus.pubsub.inngest.jobs.intake.regenerate_petition_draft_job import (
    RegeneratePetitionDraftJob,
)


def _seed_case_assessment_analysis_with_draft(
    ...
    session.add(
        PetitionDraftModel(
            ...
            precedent_citations=['Citação original'],
        )
    )
    session.add(
        CaseSummaryModel(
...
class TestRegeneratePetitionDraftJob:
    def test_should_register_inngest_function_for_petition_draft_regeneration_event(
        self,
    ) -> None:
        function = RegeneratePetitionDraftJob.handle(Inngest(app_id='test'))

        assert function._triggers == [  # noqa: SLF001
            TriggerEvent(
                event=PetitionDraftRegenerationTriggeredEvent.name,
            )
        ]
    ...
    def test_should_process_event_and_publish_finished_event_when_runtime_flow_succeeds(
        ...
        async def _mark_analysis_as_regenerating_petition_draft(_payload: Any) -> None:
            return None
        ...
        monkeypatch.setattr(
            RegeneratePetitionDraftJob,
            '_mark_analysis_as_regenerating_petition_draft',
            _mark_analysis_as_regenerating_petition_draft,
        )
        monkeypatch.setattr(
            RegeneratePetitionDraftJob,
            '_regenerate_and_persist_petition_draft',
            _regenerate_and_persist_petition_draft,
        )
        monkeypatch.setattr(InngestBroker, 'publish', _publish)

        response = inngest_runtime.post_event(
            name=PetitionDraftRegenerationTriggeredEvent.name,
            data={
                'analysis_id': analysis_id,
                'comments': '  Ajustar pedidos finais  ',
            },
        )
    ...
    def test_should_replace_petition_draft_and_finish_analysis_when_sync_flow_succeeds(
        self,
        monkeypatch: MonkeyPatch,
        sqlalchemy_session_factory: sessionmaker[Session],
    ) -> None:
        seeded_data = _seed_case_assessment_analysis_with_draft(
            sqlalchemy_session_factory
        )
        captured_workflow_calls: list[dict[str, Any]] = []
        job_module = __import__(
            'animus.pubsub.inngest.jobs.intake.regenerate_petition_draft_job',
            fromlist=['_Payload'],
        )
            ...
            def run(
                ...
                *,
                ...
                captured_workflow_calls.append(
                    {
                        'analysis_id': analysis_id,
                        'current_draft_structured_facts': current_draft.structured_facts.value,
                ...
                return PetitionDraftDto(
                    analysis_id=analysis_id,
                    structured_facts='Fatos regenerados',
                    legal_grounds='Fundamentos regenerados',
                    central_thesis='Tese regenerada',
        ...
        monkeypatch.setattr(
            AiPipe,
            'get_regenerate_petition_draft_workflow',
            staticmethod(lambda: _FakeWorkflow()),
        )

        job_class = cast('Any', RegeneratePetitionDraftJob)
        regenerate_and_persist = job_class._regenerate_and_persist_petition_draft_sync  # noqa: SLF001
        ...
        assert captured_workflow_calls == [
            {
                'analysis_id': seeded_data['analysis_id'],
                'current_draft_structured_facts': 'Fatos originais',
                'case_summary': 'Resumo objetivo do caso',
        ...
        assert persisted_petition_draft.structured_facts == 'Fatos regenerados'
        assert persisted_petition_draft.legal_grounds == 'Fundamentos regenerados'
        assert persisted_petition_draft.central_thesis == 'Tese regenerada'
