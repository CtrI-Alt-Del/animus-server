...
- `POST /intake/analyses/{analysis_id}/case-assessment-case-summaries` -> `TriggerCaseAssessmentCaseSummarizationController` -> `IntakePipe.verify_analysis_by_account_from_request(...)` -> `TriggerCaseAssessmentCaseSummarizationUseCase` -> `AnalysisDocumentsRepository` / `AnalysesRepository` / `Broker` -> `CaseAssessmentCaseSummarizationTriggeredEvent`.
