...
- `GET /intake/analyses/{analysis_id}/petition-drafts` -> `GetSecondInstanceJudgmentDraftController` -> `AuthPipe` / `DatabasePipe` -> `GetSecondInstanceJudgmentDraftUseCase` -> `AnalysesRepository` / `SecondInstanceJudgmentDraftsRepository` -> PostgreSQL (`analyses`, `second_instance_judgment_drafts`) -> `SecondInstanceJudgmentDraftDto`.
