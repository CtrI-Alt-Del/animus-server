...
- `PATCH /intake/analyses/archive` -> `ArchiveAnalysesController` -> `AuthPipe` / `DatabasePipe` -> `ArchiveAnalysesUseCase` -> `AnalisysesRepository` -> PostgreSQL (`analyses`) -> `list[AnalysisDto]`.
...
- `POST /intake/analyses/{analysis_id}/document` -> `CreateAnalysisDocumentController` -> `AuthPipe` / `DatabasePipe` / `PubSubPipe` -> `CreateAnalysisDocumentUseCase` -> `AnalysisDocumentsRepository` / `AnalisysesRepository` / `Broker` -> PostgreSQL (`analysis_documents`, `analyses`) -> `AnalysisDocumentDto`.
...
- `POST /intake/analyses/{analysis_id}/case-summaries` -> `RequestCaseSummaryController` -> `IntakePipe.verify_analysis_by_account_from_request(...)` -> `RequestCaseSummaryUseCase` -> `AnalysisDocumentsRepository` / `AnalisysesRepository` / `Broker` -> `CaseSummaryRequestedEvent` ou `SecondInstanceCaseSummarizationTriggeredEvent`, conforme `Analysis.type`.
...
- `GET /intake/analyses/{analysis_id}/first-instance-report` -> `GetFirstInstanceAnalysisReportController` -> `AuthPipe` / `DatabasePipe` -> `GetFirstInstanceAnalysisReportUseCase` -> `AnalisysesRepository` / `AnalysisDocumentsRepository` / `CaseSummariesRepository` / `AnalysisPrecedentsRepository` / `SecondInstanceJudgmentDraftsRepository` -> PostgreSQL (`analyses`, `analysis_documents`, `case_summaries`, `analysis_precedents`, `judgment_drafts`) -> `FirstInstanceAnalysisReportDto`.
