...
| `validation` | `src/animus/validation/` | Schemas Pydantic de saída reutilizáveis ou de entrada compartilhados entre contextos |
...
- **Nível de detalhe esperado em métodos:** descreva a assinatura Python (nome,
  ...
  Exemplo: `execute(name: str, email: str) -> AccountDto` — cria uma conta e
  retorna o DTO com o ID gerado; lança `AccountAlreadyExistsError` se o e-mail
  já estiver em uso.
...
Ex: "não é permitido cadastrar dois usuários com o mesmo e-mail".]
...
- **Método principal:** `execute(...) -> ...` — assinatura e responsabilidade
...
> Usar apenas para schemas de **saída reutilizáveis** entre contextos ou schemas de **entrada compartilhados** entre múltiplos controllers. Schemas de entrada exclusivos de um controller devem ser definidos como `*Body` no próprio arquivo do controller — não aqui.

- **Localização:** `src/animus/validation/<contexto>/` (**novo arquivo** se aplicável)
- **Tipo:** `ResponseSchema`
- **Atributos:** campos com tipos Pydantic

## Camada REST (Controllers)

- **Localização:** `src/animus/rest/controllers/<contexto>/` (**novo arquivo**)
- **`*Body`:** schema Pydantic de entrada definido **no mesmo arquivo do controller**; descrever campos e validações relevantes
- **Método HTTP e path:** ex: `POST /auth/sign-up`
...
- **Fluxo:** `*Body` → named params diretos **ou** `*Body.to_dto()` → `UseCase.execute()` → resposta
  - Usar **named params** quando os campos do `*Body` são primitivos simples (`str`, `int`, `bool`)
  - Usar **`to_dto()`** quando há conversão para tipos de domínio (ex: `Email`, `Name`, `Id`)

## Camada Routers

- **Localização:** `src/animus/routers/` (**novo arquivo** se aplicável)
- **Prefixo da rota:** ex: `/auth`
...
- **Localização:** `src/animus/core/<contexto>/domain/events/` (**novo arquivo** se aplicável)
- **`NAME`:** string identificadora do evento (ex: `"auth/email_verification_requested"`)
...
- Schemas `*Body` de entrada são sempre definidos **no arquivo do controller**
  que os utiliza — nunca em `validation/`. O repasse ao `UseCase` é via
  **named params** (campos primitivos) ou **`to_dto()`** (conversão para tipos
  de domínio), nunca conversão espalhada no corpo do endpoint.